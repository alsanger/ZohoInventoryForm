...
class ZohoAuthController extends Controller
{
     ...
     */
    public function redirectToZohoAuth(): RedirectResponse
    {
        $authUrl = $this->zohoAuthService->getAuthUrl();
        Log::info('Redirecting user to Zoho for authorization.', ['auth_url' => $authUrl]);
        return redirect()->away($authUrl); // Перенаправляем пользователя на Zoho Auth URL
    }
    ...
    public function handleZohoCallback(ZohoAuthCallbackRequest $request): RedirectResponse
    {
        ...
        // Предполагаем, что фронтенд будет доступен по адресу, указанному в ZOHO_FRONTEND_URL.
        // Вы должны будете добавить эту переменную в .env.
        $frontendBaseUrl = env('ZOHO_FRONTEND_URL', 'http://localhost:8080');
